...
if(!"BEYOND.env" %in% reticulate::conda_list()[,"name"]) {
  ...
  reticulate::conda_create("500.env",  packages = c("numpy","seaborn", "scikit-learn", "statsmodels", "numba", "pytables==3.6.1"))
  reticulate::conda_install("500.env", packages = c("python-igraph","leidenalg"), pip=TRUE)
  reticulate::conda_install("500.env", packages = c("scanpy","palantir","phate"), pip = TRUE)
}
...
load.metadata <- function() {
  read.csv("BEYOND/data/ROSMAP.participants.metadata.csv", header = T) %>%
    dplyr::mutate(
