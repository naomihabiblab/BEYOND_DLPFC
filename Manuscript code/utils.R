...
source("Cell-type analysis/utils/signatures.R")

# Base directory for saving figure panels 
panel.path <- "Manuscript code/graphs"

# Settings for accessing atlas data
# For details see Cell-type analysis/aggregate.cell.type.data.R
...
snuc.overlapping.donors <- data$uns$trajectories$pseudotime %>% `names<-`(data$uns$trajectories$branch.probs$index) %>% Filter(function(x) x < .1, .) %>% names
...
# Load cell-type and subpopulation information for plotting. This files specifies the order in which to plot
# different subpopulations, as well as genes and pathways whose text are to be added to plots
atlas <- openxlsx::read.xlsx("Manuscript code/data/figures.data.xlsx", sheet = 1)
atlas <- split(atlas, atlas$cell.type) %>% lapply(., function(ct)
  list(state.order = ct %>% tidyr::separate_rows(states, sep = "/") %>% pull(states),
       main.group  = ct %>% tidyr::separate_rows(states, sep = "/") %>% pull(grouping) %>% factor(., levels=unique(.)),
       sub.group   = ct %>% mutate(v=states) %>% tidyr::separate_rows(states, sep = "/", ) %>% pull(v),
       pathways    = ct %>% tidyr::separate_rows(pathways, sep="/") %>%
         filter(if_all(c(states, pathways), Negate(is.na))) %>%
         unstack(pathways~states)
  ))
...
atlas.genes <- split(atlas.genes, atlas.genes$cell.type) %>% lapply(., function(ct) 
  ct %>% tidyr::separate_rows("genes",sep="/") %>% unstack(genes~grouping))
